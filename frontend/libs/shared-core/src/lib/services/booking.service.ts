...
import { AvailabilityResponse, BookingCreateRequest, BookingCreateResponse, BookingDetailResponse, BookingListItemResponse, BookingResultRequest, BookingStatus, CancellationResponse, JoinPaymentCheckResponse, JoinRequestResponse, PublicBookingResponse, SlotResponse } from '../models/booking.models';
...
export class BookingService {
  ...
  getAvailableSlots(courtId: number, date: string): Observable<AvailabilityResponse> {
    const params = new HttpParams().set('courtId', courtId).set('date', date);
    return this.http.get<AvailabilityResponse>(`${this.apiUrl}/bookings/availability`, { params });
  }
